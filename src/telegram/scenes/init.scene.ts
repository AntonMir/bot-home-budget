...
export default (init: Scenes.BaseScene<BotContext>): void => {
    const logger: Logger = setLogger({ name: SCENES.INIT })

    init.enter(async (ctx) => {
        await ctx.scene.enter(SCENES.EXCEL)
    });

    init.action('action', async (ctx) => {
    });

    init.on('message', async (ctx, next) => {
    })

    init.hears(/.*/, async (ctx, next) => {
    });
